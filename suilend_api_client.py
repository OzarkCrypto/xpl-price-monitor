    ...
    def _parse_positions_data(self, data: Dict[str, Any]) -> Optional[WalletSummary]:
        ...
        try:
            ...
            if 'positions' in data:
                for pos_data in data['positions']:
                    position = LoanPosition(
                        ...
                        weighted_borrows=float(pos_data.get('weightedBorrows', 0)),
                        liq_threshold=float(pos_data.get('liqThreshold', 0.95))
                    )
                    positions.append(position)
                    
                    total_collateral += position.collateral_amount
                    total_borrowed += position.borrowed_amount
                    total_weighted_borrows += position.weighted_borrows
                    total_liq_threshold += position.liq_threshold
            ...
            return WalletSummary(
                ...
                positions=positions,
                total_weighted_borrows=total_weighted_borrows,
                total_liq_threshold=total_liq_threshold
            )
        ...
        try:
            ...
            mock_positions = [
                LoanPosition(
                    ...
                    health_factor=1.5,
                    weighted_borrows=950.0,  # 가중 대출 금액
                    liq_threshold=0.92       # 청산 임계값
                ),
                LoanPosition(
                    asset="USDC",
                    borrowed_amount=500.0,
                    collateral_amount=800.0,
                    ltv=0.625,
                    liquidation_threshold=0.90,
                    health_factor=1.2,
                    weighted_borrows=450.0,  # 가중 대출 금액
                    liq_threshold=0.88       # 청산 임계값
                )
            ]
            
            total_weighted_borrows = sum(pos.weighted_borrows for pos in mock_positions)
            total_liq_threshold = sum(pos.liq_threshold for pos in mock_positions)
            
            return WalletSummary(
                total_collateral_usd=2800.0,
                total_borrowed_usd=1500.0,
                total_ltv=0.536,
                health_factor=1.2,
...
def test_api_client():
    ...
    if positions:
        ...
        print(f"✅ 총 가중 대출: ${positions.total_weighted_borrows:,.2f}")
        print(f"✅ 총 청산 임계값: {positions.total_liq_threshold:.2%}")
        
        for pos in positions.positions:
            print(f"  - {pos.asset}: LTV {pos.ltv:.2%}, 헬스 {pos.health_factor:.2f}")
            print(f"    가중 대출: ${pos.weighted_borrows:,.2f}, 청산 임계값: {pos.liq_threshold:.2%}")
    else:
